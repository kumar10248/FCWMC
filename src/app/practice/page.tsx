...
export default function PracticeSelectionPage() {
  ...
  const weekNumbers = Array.from({ length: 6 }, (_, i) => i+1 );
        ...
        <div className="max-w-4xl mx-auto bg-gray-900/50 rounded-xl p-6 shadow-xl backdrop-blur-sm">
          ...
          <div className="mb-10">
            <h3 className="text-lg font-medium mb-4 pl-2 border-l-4 border-amber-400 flex items-center">
              <span className="bg-amber-400 text-black rounded-full p-1 mr-2 text-xs flex items-center justify-center w-6 h-6 font-bold">M</span>
              Weekly Practice
            </h3>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 gap-4">
              {weekNumbers.map((weekNum) => {
                const mode = `module${weekNum}` as PracticeMode;
                return (
                  <button
                    key={mode}
                  ...
                  >
                    <span className="text-3xl mb-2 font-bold">M{weekNum}</span>
                    <span className="text-sm opacity-80">Module {weekNum}</span>
                  </button>
                );
              })}
            </div>
